#include "widget_dialog_device_borrow.h"
#include "widget_dialog_signature.h"
#include "command.h"
#include "model.h"
WidgetDialogDeviceBorrow::WidgetDialogDeviceBorrow(QString title, int width, int height, QWidget *parent)
...
void WidgetDialogDeviceBorrow::exit()
{
	close();
}
void WidgetDialogDeviceBorrow::recognize()
{
...
void WidgetDialogDeviceBorrow::confirm()
{
	WidgetDialogSignature* wddSignature = new WidgetDialogSignature(0, "대출하기", 500, 540, this);
	wddSignature->setData(edBarcode->toPlainText());
	wddSignature->show();
}
void WidgetDialogDeviceBorrow::cancel()
{
	close();
}
void WidgetDialogDeviceBorrow::init()
{
