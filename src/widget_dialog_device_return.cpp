#include "widget_dialog_device_return.h"
#include "widget_dialog_signature.h"
#include "command.h"
#include "model.h"
WidgetDialogDeviceReturn::WidgetDialogDeviceReturn(QString title, int width, int height, QWidget *parent) : WidgetDialog(title, width, height, parent)
{
	setModal(true);
...
void WidgetDialogDeviceReturn::exit()
{
	close();
}
void WidgetDialogDeviceReturn::recognize()
{
...
void WidgetDialogDeviceReturn::confirm()
{
	WidgetDialogSignature* wddSignature = new WidgetDialogSignature(0, "반납하기", 500, 540, this);
	wddSignature->setData(edBarcode->toPlainText());
	wddSignature->show();
}
void WidgetDialogDeviceReturn::cancel()
{
	close();
}
void WidgetDialogDeviceReturn::init()
{
